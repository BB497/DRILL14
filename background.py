...
class TileBackground:
    ...
    def __init__(self):
        ...
        self.titles = [[load_image('cube%d%d.png' % (x, y)) for x in range(3)] for y in range(3)]
        pass
    ...
    def draw(self):
        ...
        for ty in range(title_bottom, title_top+1):
            for tx in range(title_left, title_right+1):
                self.titles[ty][tx].draw_to_origin(-left_offset + (tx-title_left)*800,
                                                   -bottom_offset+(ty-title_bottom)*600)
        pass
